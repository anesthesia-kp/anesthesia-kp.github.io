...
**STATUS — generated 2026-08-24 16:05 UTC by `node status.mjs`. Regenerate after every push.**
...
| Schedule admin / staff | 77 / 36 | 77 / 36 | not checked from here (no network) — fetch live separately |
...
Suites on disk: auction **54** (`tests/test-*.mjs`) · schedule **28**
...
> 2. **The schedule battery's skip gate is wrong** — it checks for a chromium BINARY, not the
...
>
> **STILL OPEN AND STILL NEEDING A RULING: items 2 and 3 above** — the battery's skip gate, and
> the five builds with no suite. Both are test infrastructure; neither can harm the auction.
>
...
- **5 · auto-populate loops the whole catalog — STILL OPEN.** Both `autoPopulate` and
...
- **13 · year-mode Stats in-flight guard — STILL OPEN, but narrower than recorded.** There IS a
...
- **25 · `callTotals`** — still a live per-page-load Firestore read feeding a variable with
