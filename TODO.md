...
**STATUS — generated 2026-08-19 05:20 UTC by `node status.mjs`. Regenerate after every push.**
...
Suites on disk: auction **22** (`tests/test-*.mjs`) · schedule **23**
...
| **FB-1** | **Firestore DAILY READ QUOTA is a plausible mid-auction outage, and the ever-growing `changes` log is the multiplier.** Owner question, 19 Aug 2026. Modelled from the code, NOT measured: every staff client holds ~20 live listeners, and a listener costs one document read per change delivered — so ONE bid, which writes ~4 documents every client listens to (bids · changes · bidTimes · timer), costs roughly **4 × (connected clients)** reads. At ~60 doctors that is ~240 reads per bid action; a few hundred bid actions in a busy day crosses the free tier's 50,000 reads/day. Worse, `changes` is append-only and **never trimmed** (only a manual Clear Log and archive-on-phase-complete), and the WHOLE document is re-sent to every listener on every append — so both reads and egress grow with log size × clients × changes | **OPEN — analysis only, 19 Aug 2026. Nothing measured, nothing built, no "go".** Claude cannot see the Firebase console: the PLAN, real usage and real document sizes are owner-only facts. **The live rehearsal already generated real traffic — the console's Usage graphs for those days are the ground truth and settle this in minutes** | nothing yet. Two independent questions: (a) which plan — on the free tier the daily cap DENIES reads, which would lock ~60 people out mid-phase; a paid plan with a budget alert removes the hard stop, and at this scale the spend is small; (b) the `changes` log growth, worth fixing on its own merits whatever the plan. Distinct from the already-recorded `phases` doc growth (~20KB/round vs the 1MB PER-DOCUMENT cap) |
...
> **Owner, 19 Aug 2026, when the list was put to him: "i know."** These are HIS actions, not
