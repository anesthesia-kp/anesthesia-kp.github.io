...
**STATUS — generated 2026-08-21 18:33 UTC by `node status.mjs`. Regenerate after every push.**

| | build (disk) | versions.json | live check |
|---|---|---|---|
| Auction admin / staff / mobile | 299 / 161 / — | 299 / 161 / 18 | live: unreachable (offline?) |
...
Suites on disk: auction **48** (`tests/test-*.mjs`) · schedule **27**
...
> **STATE AT THE CLOSE OF 21 Aug 2026.** §77 shipped as admin **298** and §79 as admin **299**,
> both filed and awaiting the owner's push. All four §77 questions are answered (DECISIONS
> §80/§81/§82) and the pre-commit check caught a real defect before it shipped (§82d).
>
> **The queue head is no longer a defect list.** The owner's standing decision (21 Aug) is that
> the small-item chase is endless and stops: 10 MEDIUM and the LOW/NIT tail remain listed but are
> NOT to be worked through, and the next audit looks ONLY for auction-derailing events that could
> cause real and genuine harm.
>
> **Two things genuinely outstanding:**
> 1. ⭐ **Real-bidder sign-ins** — the owner's own top item, below, and the one no audit can do.
> 2. The **21 browser-based schedule suites** were not re-run in-cloud after 299 (staging was
>    refused — stale desktop sign-in). They do not touch the auction. Re-run after signing in.
