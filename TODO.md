...
**STATUS — generated 2026-08-24 17:46 UTC by `node status.mjs`. Regenerate after every push.**
...
| Schedule admin / staff | 78 / 36 | 78 / 36 | not checked from here (no network) — fetch live separately |
...
Suites on disk: auction **54** (`tests/test-*.mjs`) · schedule **30**
...
## S-hygiene — normalise KP-address saves (fold into any next schedule admin build)

The schedule admin's Users page writes the KP address with `.trim()` and **nothing else** at all
four writers — no lower-case, not even an `@` check — into `vacations/emails`, the document the
auction has normalised since build 270. The **login** address is trim + lower-cased at all four,
because the rules match it case-sensitively; the KP address got neither. The auction lower-cases
at READ time, so the damage is masked rather than absent.
Fix = port `_normKpEmail` (trim + lower-case; refuse only the five unarguable shapes; no domain
policy — scope frozen by owner ruling 17 Aug) to `addSchedUser`, `saveSchedField` and
`saveAllSchedUsers` — **three call sites, not four: the fourth (`saveSchedUser`) was deleted in
admin 77.**
Gates when built: isolation test + schedule battery + auction battery.
