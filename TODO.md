...
**STATUS — generated 2026-08-24 18:00 UTC by `node status.mjs`. Regenerate after every push.**
...
| Schedule admin / staff | 79 / 36 | 79 / 36 | not checked from here (no network) — fetch live separately |
...
Suites on disk: auction **54** (`tests/test-*.mjs`) · schedule **31**
...
**Open — UX:**
- **15 · no withdraw — STILL OPEN.** Zero occurrences of `withdraw` or `cancel` on either page.
...
- **16 · no denial reason — STILL OPEN, and closer to done than it looks.** `decideReq` takes only
