...
**STATUS — generated 2026-08-18 16:18 UTC by `node status.mjs`. Regenerate after every push.**
...
4. ~~**S4 Storage migrations**~~ — **DONE, build 68 + staff 30** (all three lists per-item; legacy docs frozen; structural claim dedup; 24/24 + 20/20 ×3, honesty both sides, 23 suites green, auction battery green). **⚠️ DEPLOY GATE: publish the updated firestore.rules (dailysched subcollection match) to BOTH consoles BEFORE pushing these two pages — and FIRST compare the console’s current rules with the repo file: the repo file never had a dailysched subcollection match, yet builds 59/63 have needed one. If the console matches the repo file, per-day schedule writes and audit entries have been silently denied in production — check and report.**
