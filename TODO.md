...
**STATUS — generated 2026-08-18 15:46 UTC by `node status.mjs`. Regenerate after every push.**

| | build (disk) | versions.json | live check |
|---|---|---|---|
| Auction admin / staff / mobile | 269 / 139 / — | 269 / 139 / 17 | live: unreachable (offline?) |
| Schedule admin / staff | 67 / 29 | 67 / 29 | live: unreachable (offline?) |
...
Suites on disk: auction **15** (`tests/test-*.mjs`) · schedule **21**
...
4. **S4 Storage migrations** — NEXT. requests → swaps → openShifts per-item ([63] pattern). GO.
5. **S5 Grid upgrades** — coverage strip + filters + density toggle AND the flipped shifts-as-rows view (owner: "both views great"). GO.
