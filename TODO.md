...
**STATUS — generated 2026-08-24 15:49 UTC by `node status.mjs`. Regenerate after every push.**
...
| Schedule admin / staff | 76 / 36 | 76 / 36 | not checked from here (no network) — fetch live separately |
...
Suites on disk: auction **54** (`tests/test-*.mjs`) · schedule **27**
...
>
> 1. ⭐ **`saveBaseline` can erase every call-fairness baseline in one click.** Full-document
...
>    on.
>
> **Also surfaced, worth settling before anything is built on top of it:** `saveSchedUser` has had
> no call site since July 2026, yet it is one of the SIX SANCTIONED handlers on the cardinal-rule
> allowlist for writing the live auction roster. A sixth of that allowlist is dead code, so the
> guard over-states the real write surface. Deleting it shrinks the set to five with no behaviour
> change — decide it BEFORE S-hygiene writes a normaliser into unreachable code.
>
> **Nothing was built. No schedule code changed. §1's constraint is intact — the auction was not
> touched.**
...
- **7 · non-merge overwrites — SPLIT.** `clearMonth` fixed in 59 (batched, v2 marker).
  ...
  lost while the confirm sits open. **`saveBaseline` is the dangerous one — see ⚠️ 1 above.**
...
- **26 · `saveSchedUser` — zero call sites, AND IT MATTERS MORE THAN A DEAD FUNCTION SHOULD.**
  ...
  with no behaviour change. Decide this BEFORE writing S-hygiene's helper into it as a fourth
  call site.
...
`saveAllSchedUsers`. **`saveSchedUser` is the fourth writer and is DEAD — settle defect 26 first
rather than writing the helper into unreachable code.**
Gates when built: isolation test + schedule battery + auction battery.
