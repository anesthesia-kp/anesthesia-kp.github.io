...
<!-- answers: the stories behind START-HERE's rules and every paragraph retired from it, including the whole pre-pass START-HERE of 7 Sep 2026 — why a rule exists, by rule number; read when a rule's reason or history matters -->
...
---
## ⤵ moved 8 Sep 2026 from START-HERE.md — THE WHOLE PRE-PASS START-HERE (LAST REVISED 7 Sep 2026, 676 lines), verbatim

**Why the whole file is here.** The docs pass (DECISIONS §188) rewrote START-HERE as rules-and-a-map under a 350-line
tripwire. Every rule kept its binding sentence in the new file; the stories, the state paragraphs and the surrounding
narrative live here, unchanged. **To find why a rule exists: search this block for the rule's number as it was written —
`8. **A GATE CAN LIE`, `9. **ASK OF EVERY BUILD`, … `16. **AN ASSERTION THAT PINS`** — or for the ruling date. The
section numbers (§0–§7) are the same in the new file.

| in the new START-HERE | the story is under (search for) |
|---|---|
| §3 rules 1–7 (the change itself) | `## 3 · WORKING DISCIPLINE` |
| §3 rules 8–16 (what the gates taught) | `**Eight rules the gates themselves taught` |
| the commit-message cap, the outputs column, rules-file delivery, outputs hygiene | `**Commit summaries**`, `THE OUTPUTS COLUMN IS THE DELIVERABLE`, `RULES FILES GET THE SAME TREATMENT`, `OUTPUTS HYGIENE` |
| file hygiene | `**File hygiene**` |
| §4 the ritual, the bridge, git over the bridge, `tests` cannot be fetched, context | `## 4 · EVERY SESSION` |
| §6 batteries, the cloud recipe, RA-2, the cost gate, transfer fallbacks | `## 6 · RUNNING THE BATTERIES` |
| the state that stood at the top (builds 323–329, the rehearsals, RA-8/RA-10, the calendar feed, Stage 5) | the first ~60 lines below |
| the three-filesystems note; `device_commit_files` and git | `THREE FILESYSTEMS` |

# START HERE — KP East Bay Anesthesia. Both sites. The ONLY document you paste.

**LAST REVISED: 7 Sep 2026 (late evening) — §4 step 0 takes the date in America/Los_Angeles, never UTC; step 5 reports the context number. Earlier that day: admin 329 LIVE (`b0ed735`, §187 — the Complete Round backup gate, rehearsal note O-E; verified served twice; gates in `BUILD-LOG.md`). Next auction honesty baseline: admin 329 = `b0ed735`, staff 169. §186: THE FINAL CHROME REHEARSAL IS DONE (7 Sep) — four phases, three rounds, nothing broke, zero e-mails; `tests/docs/REHEARSAL-2026-09-07.md`; the live auction was LEFT in its finished state at his word (he restores from his 5:01 PM backup); O-A/O-B/O-C/O-D/O-F wait for a quiet day (`TODO.md` §1). Before that: admin 328 LIVE (`e5eea70`, §183 — RA-10's F1 + F2: a hold behind a bid is painted, counted and releasable; Struggling Users honours holds; verified served twice). §184: the tests-only fixes F3–F6 + F14 are PUSHED (`tests` `2b85dc2`); F7–F15 are DECLINED (*"skip 3"*) — do not re-raise. **NOTHING IS OPEN ON THE AUCTION; no next job is queued.**  RA-10 IS FILED (§182; `tests/docs/RA-10-2026-09-05.md`): NOTHING DAMAGED — every touched path executed against 326/168 with holds absent, zero differences; battery 77 / exit 0; both sweeps and a full phase run-through clean with a hold present. Two MEDIUM edges of the new feature and four suite holes await his rulings — `TODO.md` §1's first item; §92 stands, nothing built. admin 327 / staff 169 + rules LIVE (`1c73971`, §180/§181: withheld bid numbers — User Bids: click a green number to withhold, the grey chip to release; `vacations/bidHolds`, admin-only by rules, published to the console after his RA-2 run; RA-9 audited; verified served twice). admin 326 LIVE (`54cfc92`, §179: User Bids paint — yellow live chips, labelled legend, no Left column; verified served twice). admin 325 LIVE (`346522a`, §178: "User Bids", a read-only per-user page of bid numbers used / remaining, People section below Bid Lowerings; verified served twice, cache-busted). Its suite `test-325-user-bids` and the re-anchored `test-324-168` are pushed (`2d8164a`); every gate is the 325 row in `vacation-kp.github.io/BUILD-LOG.md`. Before that (2 Sep night): admin 324 / staff 168 LIVE (`34fc686`, §174: a round's denial binds only while its round is current; verified served twice; he re-placed the refused NP bid and it went through). His full rehearsal is DONE (§175) — this was the only finding. Earlier today: admin 323 LIVE (`b1ff467`, §172: the Current Phase card puts "✓ Complete" on its own line; verified served twice). §171: VACATION SITE ONLY, two weeks out; the hub's schedule cards are DOWN (`0aa4c92`); go-live runbook + rehearsal script in `tests/docs`. Earlier: RA-8's three builds are ALL LIVE — admin 319 / 321 / 322, staff 166 / 167 (§168: every group A–F and his O-1…O-4, each verified served twice); the walkthrough deck is updated for the Draw merge (§170). Auction admin 306–322 / staff 167 ALL LIVE (§147–§161); **RA-8 IS FILED (§166, `tests/docs/RA-8-2026-09-01.md`): nothing breaks the auction; four record defects and the rounds-as-phases class (§163/§165) await his rulings — the queue is `TODO.md` §1. His running list of findings (O-1…O-4) is there too. §164 governs every build that follows: narrow and safe, winding down.** What shipped this week is one line per build in `TODO.md` §1, the gates are in `vacation-kp.github.io/BUILD-LOG.md`, the rulings in `DECISIONS.md`. The history paragraph that stood here (every build from 88 to 318 in one breath) is in `START-HERE-ARCHIVE.md`.
...
**`tests/docs/RA-10-2026-09-05.md` (current — §182; its recommendation is the queue)**, then `RA-8-2026-09-01.md`, `RA-6-2026-08-28.md`, `RA-5-2026-08-24.md` and `RA-4-2026-08-21.md`. **Do not describe any defect by
...
0. **NAME THE SESSION — owner order, 5 Sep 2026 (§185): every session is `Vacation Auction <D Mon YYYY> V<n>`** — the date is HIS (America/Los_Angeles; `TZ=America/Los_Angeles date`), not UTC — a 7 Sep evening session was misnamed 8 Sep this way — e.g.
...
5. Report the state in a few lines — **the context reading (the number) is part of that report, every session**, per the CONTEXT paragraph below. Then work. **When in doubt during the day: run it,
