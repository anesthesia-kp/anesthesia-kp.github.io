...
**LAST REVISED: 30 Aug 2026, afternoon session (147 / 50 LIVE; §136 — the calendar-feed RULES change is FILED NOT PUSHED and NOT PUBLISHED: RA-2 → console paste → functions deploy, all his) — THE AUCTION CODE IS CLOSED BY §92: NO CHANGE TO IT WITHOUT
...
**FILED, NOT YET PUSHED OR PUBLISHED (§136, 30 Aug afternoon): `firestore.rules` — the calendar feed's two documents' READS (dailysched block only, vacations block md5-identical); RA-2 gates + fixture `28bae0c` in `tests`; `schedule/functions/DEPLOY.md` current. His order: push all four repos → double-click `RA-2.command` → paste and Publish in the console → the five deploy lines in `DEPLOY.md` → the three-row check. No page changed.** Schedule admin 147 / staff 50 (§134/§135 — the in-page dialog, the phone day list, nothing to swipe) were pushed by him (`90af11e`) and verified served twice, cache-busted, 30 Aug ~14:10 UTC; nothing auction-side changed.** 142–146 / 49 were pushed 30 Aug (`ac725ac`) and verified served twice. THE STAGE 5 RULES ARE PUBLISHED (30 Aug ~03:20 UTC, after RA-2: 136/136, honesty 46 of 46 red on `5994a1e`). **LIVE, verified
